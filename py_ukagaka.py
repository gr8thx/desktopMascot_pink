...
class Mascot:
    ...
    # 顔の差分の画像を切り替える これじゃなくても良いっぽい⁈
    #def dispImageFaceRenew(self, path):
    #    #self.canvas.delete("face") # faceタグで、顔差分を一旦、削除する。
    #    self.image_diff_renew = tk.PhotoImage(file=path)
    #    self.canvas.itemconfig("face", image=self.image_diff_renew, tag="face") # タグ付け
    ...
    def screenInit(self):
        ...
        self.dispImageBody("image/000_000.png")
    ...
    def bubbleInit(self):
        ...
        #self.dispImageBubble("image/bubble.png","Hello World !")
    ...
    def pngFileList(self):
        # 体のPathをリスト化する
        self.body_path_list = glob.glob("image/*_000.png") # 003だけ素材pngのファイル名を変更
  
        # 顔のPathを3次元リストにする
        self.face_path_list = [[[]]]

        self.face_path_list[0][0][0:0]=glob.glob("image/000_000?.png")
 
        for i in range(0,9+1+1): #000_000x～#000_009x,000_039,000_xxx
            self.face_path_list[0].append([])
        for i in range(1,9+1+1):
            self.face_path_list[0][i][0:0]=glob.glob("image/000_00"+str(i)+"*.png")
        self.face_path_list[0][10][0:0]=glob.glob("image/000_039.png") # ←飛び番？
        self.face_path_list[0][11][0:0]=glob.glob("image/000_xxx.png") # ←飛び番？

        self.face_path_list.append([]) #001_000x～001_009
        for i in range(0,8):
            self.face_path_list[1].append([])
        self.face_path_list[1][0][0:0]=glob.glob("image/001_000?.png")
        self.face_path_list[1][1][0:0]=glob.glob("image/001_001.png") # ←飛び番？
        for i in range(3,7+1):
            self.face_path_list[1][i-1][0:0]=glob.glob("image/001_00"+str(i)+"*.png") # ←飛び番？
        self.face_path_list[1][7][0:0]=glob.glob("image/001_009.png") # ←飛び番？

        self.face_path_list.append([]) #002_000x～002_009x
        for i in range(0,6):
            self.face_path_list[2].append([])
        self.face_path_list[2][0][0:0]=glob.glob("image/002_000?.png")
        self.face_path_list[2][1][0:0]=glob.glob("image/002_001.png")
        self.face_path_list[2][2][0:0]=glob.glob("image/002_003.png") # ←飛び番？
        for i in range(7,9+1):
            self.face_path_list[2][i-4][0:0]=glob.glob("image/002_00"+str(i)+"*.png")
        
        self.face_path_list.append([]) #003_001x～003_009x ここで003_000xは無い
        for i in range(0,5):
            self.face_path_list[3].append([])
        self.face_path_list[3][0][0:0]=glob.glob("image/003_001*.png")
        for i in range(6,9+1):
            self.face_path_list[3][i-5][0:0]=glob.glob("image/003_00"+str(i)+"*.png")
        
        self.face_path_list.append([]) #004_001～004_002x
        for i in range(0,2):
            self.face_path_list[4].append([])
        for i in range(0,1+1):
            self.face_path_list[4][i][0:0]=glob.glob("image/004_00"+str(i)+"*.png")
        
        self.face_path_list.append([]) #005_000x～005_008x
        for i in range(0,4):
            self.face_path_list[5].append([])
        self.face_path_list[5][0][0:0]=glob.glob("image/005_000?.png")
        self.face_path_list[5][1][0:0]=glob.glob("image/005_004*.png") # ←飛び番？
        self.face_path_list[5][2][0:0]=glob.glob("image/005_007.png") # ←飛び番？
        self.face_path_list[5][3][0:0]=glob.glob("image/005_008*.png")

        self.face_path_list.append([]) #007_000x～007_008x
        for i in range(0,7):
            self.face_path_list[6].append([])
        self.face_path_list[6][0][0:0]=glob.glob("image/007_000?.png")
        self.face_path_list[6][1][0:0]=glob.glob("image/007_001*.png")
        for i in range(3,7+1):
            self.face_path_list[6][i-1][0:0]=glob.glob("image/007_00"+str(i)+"*.png")#

        self.face_path_list.append([]) #008_000x～008_008
        for i in range(0,6):
            self.face_path_list[7].append([])
        self.face_path_list[7][0][0:0]=glob.glob("image/008_000?.png")
        self.face_path_list[7][1][0:0]=glob.glob("image/008_003*.png")
        self.face_path_list[7][2][0:0]=glob.glob("image/008_004.png")
        for i in range(6,8+1):
            self.face_path_list[7][i-3][0:0]=glob.glob("image/008_00"+str(i)+"*.png")

        #print(self.face_path_list)

    # imageフォルダ内のlines.txtの中のセリフをリストにする。
    def linesList(self):
        path = "image/lines.txt"
    ...
    def repeatChange(self):
        ...
        rand_empty = random.randint(0, 3) # 数回に一回だけセリフが無くなる
        if(rand_empty==0):
            self.dispImageBubble("","") # セリフ無し（吹き出しを消す）
        elif(rand_empty==1):
            self.dispImageBubble("image/bubble_jagged.png",self.lines_list[rand_lines])
        else:
            self.dispImageBubble("image/bubble.png",self.lines_list[rand_lines])
